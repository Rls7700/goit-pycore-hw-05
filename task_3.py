...
def parse_log_line(line:str) -> Optional[dict]:
    ...
    повертає dict або None якщо рядок неправильий
    """
...
def load_logs(file_path: str) -> List[dict]:
    ...
    logs: List[dict] = []
    try:
        # відкривіаеємо файл у режимі читання
        with open(file_path, "r", encoding = "utf-8") as f:
            for line in f:
                ...
                # якщо коректний додаємо до списку
                if parsed:
                    logs.append(parsed)
...
def filter_logs_by_level(logs: List[dict], level: str) -> List[dict]:
    ...
    # робимо рівень великими літерами
    level = level.upper()

    # функціональни йстиль filter + lambda
    # lambda перевіряє чи співпадає рівень
    return list(filter(lambda log: log.get("level") == level, logs))

def  count_logs_by_level(logs: List[dict]) -> Dict[str, int]:
    """підраховує кількість записів для кожного рівня"""
    # defaultdict - якщо ключа немає, значення автоматично буде 0
    counts: Dict[str, int] = defaultdict(int)
    for log in logs:
        counts[log["level"]] += 1

    # переводимо в звичайний словник
    return dict(counts)
...
def displsy_logs_details(level: str, logs: List[dict]) -> None:
    ...
    # проходимо по фіотрованих логах
    for log in logs:
        print(f"{log['date']} {log['time']} - {log['message']}")

def main() -> None:

    #якщо шлях не передано
    if len(sys.argv) < 2:
    ...
    # другий аргумент - рівеень логування
    level = sys.argv[2] if len(sys.argv) >= 3 else None

    # завнатжуємо все логи
    ...
    # якщо введено рівень показує деталі
    if level:
